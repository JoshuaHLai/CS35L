...
void* helper_function(void* thread){

    /*Generates warning about type conversion - ignored since results are still printed*/
    ...
    /* for every pixel */
    for( int px = num_threads; px<width; px += nthreads )
    {
...
main( int argc, char **argv )
{
    ...
    /*Create array of threads to store amount of threads used*/
    pthread_t threads[nthreads];

    /*Generate threads and distribute data to be processed among each thread*/
    int i = 0;
    int threads_created;
    while(i < nthreads)
    {
    ...
    /*To make sure that there are no race conditions that occur, leave this part outside of the previous for
    loop.  Once all data has been processed through threads, terminate threads.  Utilize different variables
    since dont understand behavior of using same variable.*/
    int j = 0;
    while(j < nthreads)
    {
        threads_created = pthread_join(threads[j], NULL);
        if(threads_created)
        {
    ...
    /*Used from original function to print out pixel colors.  Looking at the raw "*.ppm" file,
    numbers are printed out that correspond to their shade (i.e. 0 - white, 255 - black)*/
    int px = 0;
    int py = 0;       
